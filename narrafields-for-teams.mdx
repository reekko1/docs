...
NarraFields automatically extracts clinical information from your patient notes based on **YOUR** custom field definitions. 
...
<CardGroup cols={2}>
  <Card title="Define What Matters" icon="clipboard-list">
    "Track pain scores 1-10"
    
    You decide what to extract
  </Card>
  <Card title="AI Finds It" icon="robot">
    "Pain 7/10 today" → 7
...
Every practice is different. **YOUR** practice defines what's important to track.
...
### Field Types **YOUR** Practice Can Create
...
When creating each field, **YOUR** team sets three key processing behaviors:

<AccordionGroup>
  <Accordion title="Auto-Update">
    **Enabled**: AI automatically processes this field whenever new patient notes are added
    
    **Disabled**: AI only processes when you manually trigger it
    
    **Best for Auto-Update**: Vital signs, lab values, medication changes - objective data you want tracked immediately
    
    **Best for Manual**: Diagnoses, treatment plans, clinical assessments - important decisions you want to control
  </Accordion>

  <Accordion title="Overwrite Existing">
    **Enabled**: AI will update the field even if someone manually entered a value
    
    **Disabled**: AI skips fields that already have manual entries, preserving your work
    
    **Example**: Blood pressure field set to "don't overwrite" - if a nurse manually enters 120/80, AI won't change it even if new notes mention different readings
  </Accordion>

  <Accordion title="Requires Approval">
    **Enabled**: AI suggestions wait for your approval before being saved
    
    **Disabled**: AI extractions are saved immediately without review
    
    **Best for Approval**: Clinical judgments, diagnoses, risk assessments - anything requiring medical review
    
    **Best for Auto-Save**: Objective measurements, dates, clearly documented facts
  </Accordion>
</AccordionGroup>

**Real Configuration Example:**
- **Field**: "Primary Diagnosis"
- **Auto-Update**: Enabled (process when new notes added)
- **Overwrite Existing**: Disabled (preserve manual diagnoses)  
- **Requires Approval**: Enabled (medical team reviews before saving)
- **Fallback**: "If diagnosis unclear, respond with 'Diagnosis pending further evaluation'"
...
<Note>
Critical clinical information always waits for your approval. **YOUR** practice decides what requires review when setting up each field.
</Note>
...
<AccordionGroup>
  <Accordion title="Will this change how I document?">
    **No.** Continue writing notes as usual. NarraFields extracts only the information **YOUR** practice chooses to track. No new templates required.
  </Accordion>
