...
---
The FCM (Firebase Cloud Messaging) endpoint provides a comprehensive push notification system for Narra's healthcare workflows. It supports multi-device management, healthcare-specific notification templates, and clinical workflow integration for keeping medical teams connected and informed.
...
const registerFCMToken = async (fcmToken) => {
  ...
  const response = await fetch('/fcm/tokens/register', {
    method: 'POST',
    headers: {
      'X-API-Key': apiKey,
...
<ParamField body="platform_data" type="object">
  ...
  - **iOS**: `bundle_id`, `environment`
  - **Android**: `package_name`, `environment`
  - **Web**: `domain`, `browser_info`
</ParamField>
...
```json
{
  ...
  "masked_token": "fcm_token...last_chars"
}
```
...
```json
{
  "tokens": [
    {
      "masked_token": "fcm_abc123...xyz789",
      "device_type": "ios",
      "device_name": "Dr. Smith iPhone",
      "app_version": "1.2.3",
...
<ParamField body="recipient" type="object" required>
  Recipient targeting:
  - `type: "user_id"` - Send to all devices for a user
  - `type: "email"` - Find user by email and send to their devices  
  - `type: "tokens"` - Send directly to comma-separated FCM tokens
</ParamField>
...
<ParamField body="notification" type="object" required>
  ...
  - `click_action` (string, optional) - Deep link destination
</ParamField>
...
<ParamField body="priority" type="string">
  Notification priority: `high`, `normal`, or `low`
</ParamField>
...
<ResponseField name="400 Bad Request">
  Invalid template data or missing required fields
  ```json
  {
    "detail": {
      "template": "lab_results",
      "errors": ["patient_name is required", "lab_type is required"]
    }
  }
  ```
</ResponseField>
...
class FCMManager {
  ...
  async registerToken(userId: string, token: string) {
    ...
    await fetch('/fcm/tokens/register', {
      method: 'POST',
      headers: {
        'X-API-Key': apiKey,
        'Content-Type': 'application/json'
...
- **Token registration**: Verify tokens are active in `/fcm/tokens/{user_id}`
- **Firebase console**: Check FCM delivery reports
- **Template validation**: Use `/fcm/templates/schemas` for field requirements
- **Test notifications**: Use `/fcm/test/{user_id}` to verify setup
</Tip>
