...
**GET** `/meta/form-fields/{collection_type}`
...
**GET** `/meta/form-fields/vitals/{vital_type}`
...
**GET** `/meta/history-edit-fields/labs`
...
**GET** `/meta/vitals/{vital_type}/history-edit-fields`
...
**GET** `/meta/top-level-edit-fields/{collection_type}`

Returns fields that can be edited via PUT `/{doc_id}/top` endpoint.
...
<Note>
Vitals and labs return empty arrays - they don't support top-level field editing.
</Note>
...
<ResponseField name="404 Not Found">
  ...
  ```json
  {
    "detail": "Vital type 'invalid_vital' not found."
  }
  ```
</ResponseField>
...
const generateForm = async (collectionType: string) => {
  const response = await fetch(`/meta/form-fields/${collectionType}`, {
...
const getVitalFields = async (vitalType: string) => {
  const response = await fetch(`/meta/form-fields/vitals/${vitalType}`, {
    headers: { 'X-API-Key': apiKey }
  });
...
const generateEditForm = async (collectionType: string, entryType?: string) => {
  ...
  if (collectionType === 'labs') {
    endpoint = '/meta/history-edit-fields/labs';
  } else if (collectionType === 'vitals' && entryType) {
    endpoint = `/meta/vitals/${entryType}/history-edit-fields`;
  } else {
    endpoint = `/meta/top-level-edit-fields/${collectionType}`;
  }
